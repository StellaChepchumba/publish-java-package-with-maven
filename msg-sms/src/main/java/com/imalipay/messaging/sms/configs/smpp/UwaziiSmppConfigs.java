...
@Setter
@Configuration
@ConfigurationProperties(prefix = "imalipay.messaging.sms.config.smpp.uwazii") 
public class UwaziiSmppConfigs 
{
	private String name;
	private String host;
	private int port;
	...
	@Bean(name="uwaziiSmppSession", destroyMethod = "destroy")
	public SmppSession session(
		@Qualifier("uwaziiSmppSessionConfigs") SmppSessionConfiguration configs)
	...
	@Bean(name="uwaziiSmppSessionConfigs")
	...
	@Bean(name="uwaziiSmppSessionManger")
	public SmppSessionManager sessionManager(
		@Qualifier("uwaziiSmppSession") SmppSession session
	)
	{
		return new SmppSessionManager(sessionTimeout, session);
	}
	
	@Bean(name = "uwaziiSmppClient")
